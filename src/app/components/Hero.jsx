"use client";
import React from "react";
import {
  MouseParallaxContainer,
  MouseParallaxChild,
} from "react-parallax-mouse";
import Image from "next/image";
import { TypeAnimation } from "react-type-animation";
import { motion } from "framer-motion";
import { fadeIn } from "../../../variants";

const LocationSequence = [
  "Atalanta - USA",
  3000,
  "England, United Kingdom",
  3000,
  "Gold Coast, Australia",
  3000,
  "Manchester, England",
  3000,
  "Boston - USA",
  3000,
];

function Hero() {
  return (
    <section className="h-[80vh] xl:h-[850px]" id="home">
      <div className="container mx-auto h-full flex justify-center items-center xl:justify-start ">
        <div className="h-full flex flex-col justify-center items-center xl:items-start z-20 pt-12">
          <MouseParallaxContainer
            globalFactorX={0.1}
            globalFactorY={0.2}
            resetOnLeave
            className="relative flex items-center h-[120px} xl:h-max xl:w-[840px]"
          >
            <MouseParallaxChild
              factorX={1.0}
              factorY={1.0}
              className="relative"
            >
              <motion.div
                variants={fadeIn("up", 0.4)}
                initial="hidden"
                whileInView={"show"}
                viewport={{ once: "false", amount: 0.3 }}
                className="w-[300px] h-[101.37px xl:w-[725px] xl:h-[244.97px] "
              >
                <Image
                  src={"/assets/hero/davidotext.png"}
                  fill
                  alt="typo"
                  className="object-contain opacity-20"
                />
              </motion.div>
            </MouseParallaxChild>
            <MouseParallaxChild
              factorX={1.5}
              factorY={1.5}
              className="absolute xl:left-6 z-30"
            >
              <motion.div
                variants={fadeIn("up", 0.4)}
                initial="hidden"
                whileInView={"show"}
                viewport={{ once: "false", amount: 0.3 }}
                className="w-[300px] h-[101.37px xl:w-[725px] xl:h-[244.97px} "
              >
                <Image
                  src={"/assets/hero/adeleke.png"}
                  height={400}
                  width={600}
                  alt="typo"
                  className="object-contain"
                />
              </motion.div>
            </MouseParallaxChild>
            <MouseParallaxChild
              factorX={0.5}
              factorY={0.9}
              className="hidden xl:flex absolute right-0 z-20 opacity-80"
            >
              <motion.div
                variants={fadeIn("left", 0.4)}
                initial="hidden"
                whileInView={"show"}
                viewport={{ once: "false", amount: 0.3 }}
                className="w-[150px] h-[100px] xl:w-[248px] xl:h-[200px] mix-blend-luminosity px-15"
              >
                <Image
                  src={"/assets/hero/bird.png"}
                  height={200}
                  width={200}
                  alt="typo"
                  className="object-contain"
                />
              </motion.div>
            </MouseParallaxChild>
          </MouseParallaxContainer>

          <motion.div
            variants={fadeIn("up", 1)}
            initial="hidden"
            whileInView={"show"}
            viewport={{ once: "false", amount: 0.7 }}
            className="min-h-[60px] flex items-center mb-6 text-[26px]"
          >
            <div className="hiddden xl:flex items-center xl:gap-x-0">
              <div className="">World</div>
              <div className="relative w-2 h-2 mx-2 flex items-center justify-center">
                <Image fill src={"/assets/hero/dot.svg"} />
              </div>
              <div>Tour</div>
              <div className="relative w-2 h-2 mx-2 flex items-center justify-center">
                <Image fill src={"/assets/hero/dot.svg"} />
              </div>
              <div>2023</div>
            </div>
            <div className="hidden xl:flex items-center justify-center relative w-7 h-7 mx-4">
              <Image
                src={"/assets/hero/mic.svg"}
                alt="microphone"
                height={200}
                width={200}
              />
            </div>
            <TypeAnimation
              sequence={LocationSequence}
              wrapper="div"
              speed={10}
              deletionSpeed={10}
              repeat={Infinity}
              cursor={false}
            />
          </motion.div>

          <motion.div
            variants={fadeIn("up", 1.3)}
            initial="hidden"
            whileInView={"show"}
            viewport={{ once: "false", amount: 0.3 }}
          >
            <button className="btn btn-sm btn-accent">Get Tickets</button>
          </motion.div>
        </div>

        <motion.div
          variants={fadeIn("left", 0.2)}
          initial="hidden"
          whileInView={"show"}
          viewport={{ once: "false", amount: 0.7 }}
          className="hidden xl:flex absolute right-0 top-0 before:m-[784px] before:h-[893px] before:absolute before:right-0 before:top-0 before:bg-singerOverlay before:z-10"
        >
          <Image
            src={"/assets/hero/singer.jpg"}
            width={500}
            height={400}
            priority
            alt="Singer"
            quality={100}
          />
        </motion.div>
      </div>
    </section>
  );
}

export default Hero;
